...
data.role="";
	...
	{
		data.role="employee";
		data.filter = 'caller_idDYNAMIC90d1921e5f510100a9ad2572f2b477fe^company=2ccbea97db03001091f67a61399619c0';
	}
	...
	data.title = input.headerTitle || "Incidents";
	...
	while (gr._next()) {
		...
		listIncident.push(record);
		
	}
	
	var start=data.window_start;
	while(start<data.window_end && start<data.row_count)
		{
			data.list.push(listIncident[start]);
			start=start+1;
		}
