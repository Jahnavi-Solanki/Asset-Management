function ($scope, $location, $window, spUtil, amb, $http, spAriaUtil, $timeout, spNavStateManager) {
	...
	*/
	...
	$scope.go = function(table, item) {
		...
		item.selected = true;
		//$scope.$broadcast('incident_selected', parms);
		//$scope.$emit('incident_selected', parms);
		$window.location.href="/asm?view=sp&id=incident_detail&table=incident&sys_id="+parms.sys_id;
		}, function() {
