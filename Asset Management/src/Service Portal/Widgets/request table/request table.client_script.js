function ($scope, $location, $window, spUtil, amb, $http, spAriaUtil, $timeout, spNavStateManager) {
	...
	*/
	...
	c.assignAsset=function(item,assetId)
	{
		c.server.get({
			action:"assignAsset",
			...
			AssetId:assetId
				...
				//var row = $scope.data.list[i];
				if (c.data.list[i].sys_id ==item.sys_id)
				{
						c.data.list[i].request_state.value="closed_complete";
						c.data.list[i].request_state.display_value="Closed Complete";
						//c.data.list[i].request_state="ddone";
						//c.data.list[i]=r.data.temp;
	...
	c.rejectRequest=function(item)
	{
		c.server.get({
			...
			recordId:item.sys_id
				...
				if (c.data.list[i].sys_id ==item.sys_id)
				{
						c.data.list[i].request_state.value="closed_rejected";
						c.data.list[i].request_state.display_value="Closed Rejected";
						//c.data.list[i].request_state="ddone";
						//c.data.list[i]=r.data.temp;
	...
	c.approveRequest=function(item)
	{
		console.log("reached"+item.sys_id);
		c.server.get({
			...
			for(var i=0;i<c.data.list.length; i++) {
				...
				if (c.data.list[i].sys_id ==item.sys_id)
				{
						c.data.list[i].request_state.value="in_process";
						c.data.list[i].request_state.display_value="Approved";
						//c.data.list[i].request_state="ddone";
						//c.data.list[i]=r.data.temp;
	...
	c.rejectRequest=function(item)
	{
		...
		c.server.get({
			...
			for(var i=0;i<c.data.list.length; i++) {
				...
				{
						c.data.list[i].request_state.value="closed_rejected";
						c.data.list[i].request_state.display_value="Rejected";
						//c.data.list[i].request_state="ddone";
						//c.data.list[i]=r.data.temp;
	...
	$scope.go = function(table, item) {
		...
		spNavStateManager.onRecordChange(table).then(function() {
			...
			//$scope.$emit(eventNames.click, parms);
					$window.location.href="/asm?view=sp&id=request_detail&table=sc_request&sys_id="+parms.sys_id;
		}, function() {
